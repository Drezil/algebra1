...
\item $R = \mathbb{Z}, p $ Primzahl, $S \coloneqq \mathbb{Z} \setminus (p) \rightsquigarrow S^{-1}R =$\menge{\frac{a}{b}}{a \in \mathbb{Z}, p \nmid b}
\item $R = K[X], K$ ist ein Körper, $Q(K[X]) = K(X) =$\menge{\frac{f(x)}{g(x)}}{f,g \in K[X], g neq 0}\\
\glqq Körper der rationalen Funktionen in $X$ über $K$\grqq
\end{enumerate}

\textbf{z.B.:} $f = X^2 + 1$. Wir wollen \glqq $f = 0$\grqq\ lösen. Gegeben Ring $R$, bestimme \menge{x \in R}{f(x) = 0}\\
$\mathbb{Q}(i) = \mathbb{Q}(\sqrt{-1}) =$\menge{a + bi}{a,b \in \mathbb{Q}}\\
...
\begin{defi}[2.35]
$R,P$ wie oben. $f(X) = \sum_{i=0}^{\infty} a_I X^i \in Q(R)[X]$. Für $p \in P$ setze $v_p(f) \coloneqq$ min \menge{v_p(a_i)}{i \in \mathbb{N_0}} $\in \mathbb{Z} \cup \{\infty\}$. $f \in R[X]$ heißt \emph{primitiv}, falls $v_p(f) = 0$ für alle $p \in P$. 
...
$\ker (\varphi_p) = $\menge{f \in R[X]}{ v_p(f) > 0} (per Definition von $v_p(f)$!)
